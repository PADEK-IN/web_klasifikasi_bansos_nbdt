...
def validasi():
    if request.method == "GET":
        return render_template("pages/klasifikasi/form.jinja", name="Admin")
    elif request.method == "POST":
        res = klasifikasi_controller.create()
        if not res:
            return render_template("pages/error/500.jinja", name="Admin", message="Maaf, server error - gagal menyimpan data")
        nik = request.form.get("nik")
        encrypted_nik = hash.encrypt_nik(nik)
        return redirect(f'/klasifikasi/result/{encrypted_nik}')
    else:
        return render_template("pages/error/400.jinja", name="Admin")
...
def result(id):
    ...
    return render_template("pages/klasifikasi/result.jinja", name="Admin", data=predict)
...
def reportnb():
    ...
    return render_template("pages/klasifikasi/report.jinja", name="Admin", nb=dataNb, dt=dataDt)